...
As for the personalized prioritization of requirements: The user can see directly in the Eclipse IDE, in a View, which requirements would fit not only to his/her own interests but would also be of 
...
As for the newcomer bugs: A microservice is called which identifies bugs which are suitable for newcomers.

As for the most discussed bugs: Users interested in what bugs are the most discussed in the Eclipse community, can use the ‘Most discussed bugs’ feature which will show them a view 
containing the bugs with the largest amount of comments for the current month for a specified Product - Component combination.
